...
sampahs = ['plastik', 'kertas', 'kardus', 'kaca', 'kayu', 'logam',
           'botol kaca', 'botol plastik', 'baterai', 'tas plastik',
           'piring plastik', 'koran', 'kaleng besi', 'tutup botol',
           'majalah'
           ]
...
async def cek_sampah(ctx):
    ...
    if message.lower() in sampahs:
        await ctx.send('Sampah tersebut harus di daur ulang, berikut tips untuk daur ulang sampah:')
        await ctx.send('https://umsu.ac.id/berita/daur-ulang-sampah-pengertian-manfaat-dan-cara/')
    else: 
        await ctx.send('Sampah tersebut bisa dimusnahkan atau dibuang dengan bijak, berikut ini tips untuk membuang sampah degan bijak:')
        await ctx.send('https://citarumharum.jabarprov.go.id/mari-kelola-sampah-dengan-bijak-mulai-hari-ini/')
bot.run("token")
